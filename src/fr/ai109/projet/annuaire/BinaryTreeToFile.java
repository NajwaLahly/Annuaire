...
public class BinaryTreeToFile {
	...
	public long[] findParent(long childPos, RandomAccessFile raf, Trainee trainee) {
		Trainee parent = new Trainee();
		Trainee current = new Trainee();
		String[] resultatReadChildInFile = new String[2];
		String[] resultatReadCurrentInFile = new String[2];
		long[] parentAndChildPos = new long[2];
		resultatReadChildInFile = readTraineeInDestFile(raf, childPos);
		setRoot(raf);
		current = root;
		long idxEndCurrent = idxEndRoot;
		
		long posCurrentChildLong = 0;
		long idxStartCurrent = 0;
		long idxStartParent = 0;

		try {
			while(posCurrentChildLong != childPos) {
				parent = current;
				idxStartParent = idxStartCurrent;
				if(current.toString().compareToIgnoreCase(resultatReadChildInFile[0]) < 0) {
					//current = current.right;
					posCurrentChildLong = readCurrentChildPos(raf, idxEndCurrent, nbreByteToRead);
					parentAndChildPos[1] = nbreByteToRead;
				}
				else if(current.toString().compareToIgnoreCase(resultatReadChildInFile[0]) > 0)  {
					posCurrentChildLong = readCurrentChildPos(raf, idxEndCurrent, 0);
					parentAndChildPos[1] = 0;

				}
				
				//read right/left child object
				idxStartCurrent = posCurrentChildLong;
				resultatReadCurrentInFile = readTraineeInDestFile(raf, posCurrentChildLong);
				String CurrentChildString = resultatReadCurrentInFile[0];
				idxEndCurrent = Long.parseLong(resultatReadCurrentInFile[1]);
				//current = current.R/L
				current = trainee.stringToObject(CurrentChildString);

			}
		parentAndChildPos[0] = idxStartParent;
		System.out.println(parentAndChildPos[0]);
		System.out.println(parentAndChildPos[1]);

		}catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return parentAndChildPos;
	}	
